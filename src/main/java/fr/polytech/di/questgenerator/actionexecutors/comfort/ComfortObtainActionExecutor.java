...
public class ComfortObtainActionExecutor implements ActionExecutor
{
	...
	{
		XMLStringObjectiveElement objectiveObject = DataHandler.getRandomFromCategories("object/luxury/*");
		XMLStringObjectiveElement objectivePNJ = DataHandler.getRandomFromCategories("pnj/being/*");

		Action actionGet = new Action(this.getClass(), depth, ActionType.GET, buildObjective(objectives, new ObjectiveHelper(OBJ_GET, objectiveObject), new ObjectiveHelper(LOC_OBJECTIVE, DataHandler.getRandomFromCategories("pnj/being/*", "area/wild/*"))));
		...
		return new Quest(getSentence("Comfort_Obtain", objectivePNJ, objectiveObject), actionGet, actionGoto, actionGive);
	}
}
